...
struct ArrayPacket_Struct
{
	Packet** apacket;
	uint32 npacket;
	uint32 Size;
};
typedef CTypedPtrList<CPtrList, ArrayPacket_Struct*> CPacketList;
#else
...
private:
	...
	*/
	static void CreateStandardPackets(byte* pbyData, uint64 uStartOffset, uint64 uEndOffset, bool bFromPF, CPacketList& rOutPackedList, const uchar* pucMD4FileHash, CUpDownClient* pClient);
	static void CreatePackedPackets(byte* pbyData, uint64 uStartOffset, uint64 uEndOffset, bool bFromPF, CPacketList& rOutPackedList, const uchar* pucMD4FileHash, CUpDownClient* pClient);
	// khaos::kmod-
