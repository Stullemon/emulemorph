...
uint8 CPartFile::LoadPartFile(LPCTSTR in_directory,LPCTSTR in_filename, bool getsizeonly)
{
	...
	try{
		...
		if (GetHashCount() != GetED2KPartCount()){	// SLUGFILLER: SafeHash - use GetED2KPartCount
			...
			hashsetneeded = false;
			for (int i = 0; i < hashlist.GetSize(); i++){
				if (i < GetPartCount() && IsComplete(i*PARTSIZE, (i + 1)*PARTSIZE - 1, false/*true*/)){ //no need to check as there is nothing to check
					SetStatus(PS_READY);
					break;
				}
			}
...
//				m_nCompleteSourcesCount;
	...
	UpdatePowerShareLimit(m_nVirtualCompleteSourcesCount<=1, iCompleteSourcesCountInfoReceived>1 && (lastseencomplete!=NULL || m_nCompleteSourcesCountLow==1) && m_nVirtualCompleteSourcesCount==1,m_nCompleteSourcesCountHi>((GetPowerShareLimit()>=0)?GetPowerShareLimit():thePrefs.GetPowerShareLimit()));
	//MORPH END   - Added by SiRoB, Avoid misusing of powersharing
								...
								const CUpDownClient* client)
{
	...
	for (POSITION pos1 = gaplist.GetHeadPosition();pos1 != NULL;){
		...
		if(client) //MORPH - Added by SiRoB, Import Parts [SR13]
