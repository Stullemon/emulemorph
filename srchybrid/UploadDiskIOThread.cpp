	...
	try{
               ...
               (addedPayloadQueueSession <= nCurQueueSessionPayloadUp || addedPayloadQueueSession - nCurQueueSessionPayloadUp < nBufferLimit))
		{
			...
			if (pOpenOvFileStruct == NULL)
			{
				...
				*/
				pOpenOvFileStruct->bCompress = ShouldCompressBasedOnFilename(fullname) && thePrefs.m_bUseCompression;
				//Xman end
			...
			// Remark: At this point we have only initiated the reading operating. It may fail and so may
			// the subsequent packet generation and transmission. Anyhow, since we presume the block done
			// right here we can just as well update the SC_XFER bit in the m_abyUpPartStatus map.
			// I (Stulle) am reasonably sure that there should be no race conditions in this place because
			// we got a lock on the client as well as the blocklist and the only place where we read this 
			// particular bit (SC_XFER) is in a place that also holds a lock on the blocklist.
			// As for IncCurrentChunks and m_uiCurrentChunks: There might be a race condition there but
			// since we only read in CUpDownClient::GetCompletedPercent this is acceptable (I hope).
			uint32 start = (uint32)(pstructOverlappedEx->uStartOffset / PARTSIZE);
			UINT lastChunk = pUploadClientStruct->m_pClient->GetLastChunk();
			if (lastChunk != (UINT)-1 && start != lastChunk) {
				if (!(pUploadClientStruct->m_pClient->m_abyUpPartStatus[lastChunk] & SC_XFER))
					pUploadClientStruct->m_pClient->IncCurrentChunks(); //because client switched to new chunk we guess the former completed
				pUploadClientStruct->m_pClient->m_abyUpPartStatus[lastChunk] |= SC_XFER;
			}
			pUploadClientStruct->m_pClient->SetLastChunk(start);
	...
	if (m_bRun)
	{
		...
		if (!bError)
		{
				...
				*/
				...
				else if (bUseCompression)
				{
					CreatePackedPackets(pOverlappedExStruct->pBuffer, pOverlappedExStruct->uStartOffset
						...
						*/
						, pOverlappedExStruct->pFileStruct->ucMD4FileHash, pOverlappedExStruct->pUploadClientStruct->m_pClient);
					// khaos::kmod-
				}
				else
				{
			...
			while (!packetsList.IsEmpty() && pSocket != NULL)
			{
				...
				ArrayPacket_Struct* arrPack = packetsList.RemoveHead();
				pSocket->SendPacket(arrPack->apacket, arrPack->npacket, true, false, arrPack->Size);
				delete[] arrPack->apacket;
				delete arrPack;
#else
			...
			// Remarks: The call of AddBlockTransferred had to be moved from CSatisticFile::AddTransferred to here
			// because we won't have the chance to know which block is being transferred later. We call it here 
			// basically how we used to call it prior to 0.50b.
			// Locking again here is also not ideal because it takes up resources. Anyhow, better be thread safe.
			CSingleLock lockSharedFiles(&theApp.sharedfiles->m_mutWriteList, TRUE);
			CKnownFile* srcfile = theApp.sharedfiles->GetFileByID(pOverlappedExStruct->pFileStruct->ucMD4FileHash);
			if (srcfile != NULL)
				srcfile->statistic.AddBlockTransferred(pOverlappedExStruct->uStartOffset, pOverlappedExStruct->uEndOffset/*FIX?+1*/, 1);
			else
				ASSERT(false);
			lockSharedFiles.Unlock();
			srcfile = NULL; // not safe anymore
...
void CUploadDiskIOThread::CreateStandardPackets(byte* pbyData, uint64 uStartOffset, uint64 uEndOffset, bool bFromPF, CPacketList& rOutPacketList, const uchar* pucMD4FileHash, CUpDownClient* pClient)
{
	uint32 nPacketSize;
	uint32 togo = (uint32)(uEndOffset - uStartOffset);
	CMemFile memfile((BYTE*)pbyData, togo);
	// khaos::kmod+ Show Compression by Tarod
	pClient->notcompressed += togo;
	CString strDbgClientInfo;
	if (thePrefs.GetDebugClientTCPLevel() > 0)
		strDbgClientInfo = pClient->DbgGetClientInfo(true);
	// khaos::kmod-

#if !defined DONT_USE_SOCKET_BUFFERING
	uint32 splittingsize = max(pClient->GetDatarateAVG(), 10240); //MORPH - Determine Remote Speed
...
#endif
#if !defined DONT_USE_SEND_ARRAY_PACKET
	ArrayPacket_Struct* arrPack = new ArrayPacket_Struct;
	arrPack->Size = togo;
	arrPack->npacket = 0;
	//Packet* apacket[EMBLOCKSIZE*3/10240];
	arrPack->apacket = new Packet * [togo / nPacketSize];
#endif
	while (togo){
...
#if !defined DONT_USE_SEND_ARRAY_PACKET
		arrPack->apacket[arrPack->npacket++] = packet;
#else
...
#if !defined DONT_USE_SEND_ARRAY_PACKET
	if(arrPack->npacket)
		rOutPacketList.AddTail(arrPack);
	else
	{
		delete[] arrPack->apacket;
		delete arrPack;
...
void CUploadDiskIOThread::CreatePackedPackets(byte* pbyData, uint64 uStartOffset, uint64 uEndOffset, bool bFromPF, CPacketList& rOutPacketList, const uchar* pucMD4FileHash, CUpDownClient* pClient)
{
	uint32 togo = (uint32)(uEndOffset - uStartOffset);
	...
	*/
	UINT result = compress2(output, &newsize, pbyData, togo, thePrefs.m_iCompressLevel);
	...
	if (result != Z_OK || togo <= newsize){
		...
		*/
		CreateStandardPackets(pbyData, uStartOffset, uEndOffset, bFromPF, rOutPacketList, pucMD4FileHash, pClient);
		// khaos::kmod-
		return;
	}
	// khaos::kmod+ Show Compression by Tarod
	pClient->notcompressed += togo;
	pClient->compressiongain += (togo - newsize);
	CString strDbgClientInfo;
	if (thePrefs.GetDebugClientTCPLevel() > 0)
		strDbgClientInfo = pClient->DbgGetClientInfo(true);
	// khaos::kmod-
    ...
    while (togo){
		...
		/*MORPH - Official FIX*///theStats.AddUpDataOverheadFileRequest(24);
