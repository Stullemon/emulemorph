...
void CUploadQueue::Process() {
	...
	*/
	...
	while((UINT)avarage_tick_list.GetCount() > 300){
		avarage_dr_list.RemoveHead();
		avarage_overhead_dr_list.RemoveHead(); //MORPH - Added by SiRoB, Upload OverHead from uploadbandwidththrottler
	...
	if (avarage_tick_list.GetCount() > 1){
		...
		datarate_USS = (UINT)(1000U * (avarage_dr_list.GetTail()-avarage_dr_list.GetAt(avarage_dr_list.FindIndex(index))) / (avarage_tick_list.GetTail() - avarage_tick_list.GetHead()+1));//MORPH - Added by SiRoB, Keep An average datarate value for USS system
...
bool CUploadQueue::RemoveFromUploadQueue(CUpDownClient* client, LPCTSTR pszReason, bool updatewindow, bool earlyabort){
	...
	if(foundPos != NULL) {
		...
		*/
		...
		//MORPH END   - Adde by SiRoB, Optimization requpfile
		if (thePrefs.GetLogUlDlEvents())
             AddDebugLogLine(DLP_VERYLOW, true,_T("Removing client from upload list: %s Client: %s Transfered: %s SessionUp: %s QueueSessionUp: %s QueueSessionPayload: %s File: %s"), pszReason==NULL ? _T("") : pszReason, client->DbgGetClientInfo(), CastSecondsToHM( client->GetUpStartTimeDelay()/1000), CastItoXBytes(client->GetSessionUp(), false, false), CastItoXBytes(client->GetQueueSessionUp(), false, false), CastItoXBytes(client->GetQueueSessionPayloadUp(), false, false), (requestedFile)?requestedFile->GetFileName():_T(""));
       	client->m_dwWouldHaveGottenUploadSlotIfNotLowIdTick = 0;
