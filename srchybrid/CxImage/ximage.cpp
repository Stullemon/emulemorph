...
////////////////////////////////////////////////////////////////////////////////
/**
 * (this) points to the same pDib owned by (*src), the image remains in (*src)
...
void CxImage::Ghost(const CxImage *src)
{
	if (src){
		...
		memcpy(&info,&src->info,sizeof(CXIMAGEINFO));
		pDib = src->pDib;
		pSelection = src->pSelection;
		pAlpha = src->pAlpha;
		ppLayers = src->ppLayers;
		ppFrames = src->ppFrames;
		info.pGhost = const_cast<CxImage*>(src);
	}
}
////////////////////////////////////////////////////////////////////////////////
/**
 * turns a 16 or 32 bit bitfield image into an RGB image
 */
void CxImage::Bitfield2RGB(const uint8_t *src, uint32_t redmask, uint32_t greenmask, uint32_t bluemask, uint8_t bpp) const
{
	switch (bpp){
	case 16:
	{
 ...
 * \param bFlipImage: tune this parameter if the image is upside down
 * \return true if everything is OK
 */
