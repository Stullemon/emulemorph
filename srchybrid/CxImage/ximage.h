 ...
 * @{ */
	...
	bool	IsSamePalette(const CxImage &img, bool bCheckAlpha = true) const;
...
/** \addtogroup Pixel */ //@{
	...
	bool	SetRectColor(const RECT& rect, RGBQUAD color, bool bSetAlpha = false);
//@}
...
/** \addtogroup DSP */ //@{
	...
	void Mix(const CxImage & imgsrc2, ImageOpType op, int32_t lXOffset = 0, int32_t lYOffset = 0, bool bMixAlpha = false);
	void MixFrom(const CxImage & imagesrc2, int32_t lXOffset, int32_t lYOffset);
...
/** \addtogroup Alpha */ //@{
	...
	bool AlphaMirror();
	bool AlphaFlip();
	bool AlphaCopy(const CxImage &from);
	bool AlphaSplit(CxImage *dest);
	void AlphaStrip();
	void AlphaSet(uint8_t level);
	bool AlphaSet(const CxImage &from);
...
/** \addtogroup Protected */ //@{
	...
	void Bitfield2RGB(const uint8_t *src, uint32_t redmask, uint32_t greenmask, uint32_t bluemask, uint8_t bpp) const;
