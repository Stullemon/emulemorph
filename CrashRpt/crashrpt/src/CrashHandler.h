...
// Support for exluding file, flag added
struct FileInfo {
	CString desc;
	bool include;
	// For implicit conversion of TStrStrPair -> TStrFileInfoPair
	// (i.e. conversion CString -> FileInfo)
	FileInfo(CString d, bool i=true) : desc(d), include(i) {};
};

typedef std::pair<CString,FileInfo> TStrFileInfoPair;
typedef std::vector<TStrFileInfoPair> TStrFileInfoVector;
...
protected:
   ...
   TStrFileInfoVector                 m_files;          // custom files to add
   ...
   LCID							 m_langID;			// Support for localisation
};
