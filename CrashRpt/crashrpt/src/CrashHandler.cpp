...
void CCrashHandler::AddFile(LPCTSTR lpFile, LPCTSTR lpDesc)
{
   ...
   if (hFile != INVALID_HANDLE_VALUE)
   {
	  ...
	  // Support for excluding files, description now
	  // stored in a struct
	  FileInfo fi(lpDesc);
	  m_files.push_back(TStrFileInfoPair(lpFile, fi));
	  ::CloseHandle(hFile);
   }
}
...
{
	TStrFileInfoVector::iterator iter;
...
DWORD WINAPI CCrashHandler::DialogThreadExecute(LPVOID pParam)
{
   ...
   // Set locale of this thread to locale of main-thread (saved previously)
   SetThreadLocale(self->m_langID);
   ...
   while(status == IDCANCEL && MessageBox(NULL, CUtility::GetResString(IDS_ABORTSTRING), "Information", MB_RETRYCANCEL) == IDRETRY)
		status = mainDlg.DoModal();

   // add report files to zip
   // (moved to after the call of mainDlg to support
   // excluding files)
   TStrFileInfoVector::iterator cur = self->m_files.begin();
   for (cur = self->m_files.begin(); cur != self->m_files.end(); cur++)
      if((*cur).second.include) zlib.AddFile((*cur).first);
		// (Only include files with flag set, default on)
   ...
   if (IDOK == status || IDC_SAVE == status)
   {
          ...
          !self->MailReport(*self->m_rpt, sTempFileName, mainDlg.m_sEmail, mainDlg.m_sDescription))
      {
		 ...
		 if(self->m_sTo.IsEmpty() && !self->m_sSubject.IsEmpty())
			 MessageBox(NULL, self->m_sSubject, "Information", MB_OK);
      }
   }
   // uninitialize
   _Module.Term();
   ...
   ::ExitThread(0);
   // keep compiler happy.
//   return TRUE; // my compiler is happier without it ;-)
}
...
BOOL CCrashHandler::GenerateErrorReport(PEXCEPTION_POINTERS pExInfo, BSTR message)
{
   ...
   TStrFileInfoVector     save_m_files = m_files;
	char temp[_MAX_PATH];

	GetTempPath(sizeof temp, temp);

	// Save locale (needed to set locale in new thread)
   ...
   if (m_sTo.IsEmpty()) {
	   ...
	   if (hFile != INVALID_HANDLE_VALUE)
	   {
		   ...
		   ::WriteFile(hFile, description, sizeof(description)-1, &writtenBytes, NULL);
		   ::CloseHandle(hFile);
		   FileInfo fi(LoadResourceString(IDS_USER_DATA));
		   m_files.push_back(TStrFileInfoPair(m_userDataFile, fi));
	   } else {
		   m_userDataFile = "";
	   }
   }
