...
LVCOLUMN _ListColumns[] =
{
   ...
   */
   ...
   {  // Column 4: File size
      LVCF_FMT | LVCF_ORDER | LVCF_SUBITEM | LVCF_TEXT | LVCF_WIDTH, 
      LVCFMT_RIGHT, 
      80,				// resized, make room for scrollbar
      (LPTSTR)IDS_SIZE, 
      0, 
      3, 
      0, 
...
//   TStrStrVector  *m_pUDFiles;      // File <name,desc>
   ...
   CImageList  m_iconList;       // Shell icon list
	...
	BEGIN_MSG_MAP(CDetailDlg)
	  ...
	  NOTIFY_HANDLER(IDC_FILE_LIST, LVN_KEYDOWN, OnLvnKeydownFileList)
	END_MSG_MAP()
...
//      if (sysMenu.IsMenu())
      ...
      for (i = 0, p = m_pUDFiles->begin(); p != m_pUDFiles->end(); p++, i++)
      {
         ...
         // Description
         list.SetItemText(i, 1, p->second.desc);
		 ...
		 if (GetFileAttributesEx(path, GetFileExInfoStandard, &infoData)) {
			 list.SetItemData(i, (DWORD) infoData.nFileSizeLow);
			 if(p->second.include) {
				 // Correct unit (before: display in bytes, unit = KB)
				if(infoData.nFileSizeLow > 2048) {
					sSize.Format(TEXT("%d KB"), infoData.nFileSizeLow / 1024);
				} else {
					sSize.Format(TEXT("%d B"), infoData.nFileSizeLow);
				}
			 } else sSize = "excluded";
   ...
   void SelectItem(int iItem)
   {
      ...
      ::SetWindowText(GetDlgItem(IDC_DESCRIPTION), m_pUDFiles->at(iItem).second.desc);
   ...
   // Support for excluding files
   LRESULT OnLvnKeydownFileList(int /*idCtrl*/, LPNMHDR pNMHDR, BOOL& /*bHandled*/) {
		LPNMLVKEYDOWN lpKD = (LPNMLVKEYDOWN) pNMHDR;
		CListViewCtrl list;
		LVITEM lvItem;
		if(lpKD->wVKey == VK_SPACE) {
			list=GetDlgItem(IDC_FILE_LIST);
			if(list.GetSelectedItem(&lvItem)) {
				TStrFileInfoPair & myInfo = m_pUDFiles->at(lvItem.iItem);
				if(myInfo.second.include) {
					list.SetItemText(lvItem.iItem, 3, "excluded");
					myInfo.second.include = false;
				} else {
					CString buffer;
					DWORD size = (DWORD) list.GetItemData(lvItem.iItem);
					if(size > 2048) {
						buffer.Format(TEXT("%d KB"), size / 1024);
					} else {
						buffer.Format(TEXT("%d B"), size);
					}
					list.SetItemText(lvItem.iItem, 3, buffer);
					myInfo.second.include = true;
				}	
			}
		}
		return FALSE;
